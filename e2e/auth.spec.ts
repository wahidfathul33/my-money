...
async function seedSessionUser(opts: { onboarded: boolean }) {
  const userId = uuidv7();
 ...
 * (the reason `useSecureCookies: true` doesn't break local dev in the real
 * app — see src/lib/auth/options.ts) isn't honored by CDP's
 * `Storage.setCookies` when a `url` is supplied. Passing `domain`/`path`
 * instead of `url` avoids the scheme check and stores it as a real
 * `Secure`, `__Secure-`-prefixed cookie against `localhost`.
 */
async function setSessionCookie(context: BrowserContext, sessionToken: string): Promise<void> {
...
async function sessionExists(sessionToken: string): Promise<boolean> {
