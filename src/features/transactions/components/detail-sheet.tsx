 ...
 * amount, "A → B" meta line, and NEITHER "Edit" nor "Hapus" —
 * `voidTransaction`/`updateTransaction` (src/lib/services/transactions.ts)
 * explicitly refuse `type: 'transfer'` rows (task 08's territory, not
 * landed in this worktree), so offering either action here would just fail
 * every time.
 *
...
function DetailContent({ item, onEdit, onClose }: DetailContentProps) {
  ...
  function handleDelete() {
    startTransition(async () => {
      const result = await voidTransactionAction(item.id);
      ...
      toast.show({
        ...
        action: {
          ...
          onClick: () => {
            startTransition(async () => {
              await unvoidTransactionAction(item.id);
              router.refresh();
            });
          },
        },
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isTransfer ? (
        <p className="text-text-subtle text-xs">Transfer belum dapat diedit atau dihapus dari sini.</p>
      ) : (
